  ...
  return (
    <Layout>
      ...
      <form method="post" action="/api/auth/callback/credentials" className="mt-8 flex flex-col gap-7 justify-center w-1/3">
        ...
        <button type="submit" className="inline-block shrink-0 rounded-md bg-emerald-800 py-1.5 text-sm font-semibold text-slate-300 transition-all esae-in duration-300 hover:bg-slate-300 hover:text-emerald-800 hover:font-bold focus:outline-none active:outline-none">Sign in</button>
      </form>
    </Layout>
  );
}
